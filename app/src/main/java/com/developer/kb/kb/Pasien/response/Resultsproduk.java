
package com.developer.kb.kb.Pasien.response;

import com.google.gson.annotations.SerializedName;


public class Resultsproduk {

    @SerializedName("alamat")
    private String mAlamat;
    @SerializedName("gakin")
    private String mGakin;
    @SerializedName("hamil")
    private String mHamil;
    @SerializedName("id_pas")
    private String mIdPas;
    @SerializedName("id_pasien")
    private String mIdPasien;
    @SerializedName("id_pos")
    private String mIdPos;
    @SerializedName("jumlah_anak")
    private String mJumlahAnak;
    @SerializedName("metode")
    private String mMetode;
    @SerializedName("nama")
    private String mNama;
    @SerializedName("nama_pos")
    private String mNamaPos;
    @SerializedName("no_telp")
    private String mNoTelp;
    @SerializedName("pekerjaan")
    private String mPekerjaan;
    @SerializedName("pendidikan")
    private String mPendidikan;
    @SerializedName("pus4t")
    private String mPus4t;
    @SerializedName("tgl_lahir")
    private String mTglLahir;
    @SerializedName("umur")
    private String mUmur;

    public String getAlamat() {
        return mAlamat;
    }

    public void setAlamat(String alamat) {
        mAlamat = alamat;
    }

    public String getGakin() {
        return mGakin;
    }

    public void setGakin(String gakin) {
        mGakin = gakin;
    }

    public String getHamil() {
        return mHamil;
    }

    public void setHamil(String hamil) {
        mHamil = hamil;
    }

    public String getIdPas() {
        return mIdPas;
    }

    public void setIdPas(String idPas) {
        mIdPas = idPas;
    }

    public String getIdPasien() {
        return mIdPasien;
    }

    public void setIdPasien(String idPasien) {
        mIdPasien = idPasien;
    }

    public String getIdPos() {
        return mIdPos;
    }

    public void setIdPos(String idPos) {
        mIdPos = idPos;
    }

    public String getJumlahAnak() {
        return mJumlahAnak;
    }

    public void setJumlahAnak(String jumlahAnak) {
        mJumlahAnak = jumlahAnak;
    }

    public String getMetode() {
        return mMetode;
    }

    public void setMetode(String metode) {
        mMetode = metode;
    }

    public String getNama() {
        return mNama;
    }

    public void setNama(String nama) {
        mNama = nama;
    }

    public String getNamaPos() {
        return mNamaPos;
    }

    public void setNamaPos(String namaPos) {
        mNamaPos = namaPos;
    }

    public String getNoTelp() {
        return mNoTelp;
    }

    public void setNoTelp(String noTelp) {
        mNoTelp = noTelp;
    }

    public String getPekerjaan() {
        return mPekerjaan;
    }

    public void setPekerjaan(String pekerjaan) {
        mPekerjaan = pekerjaan;
    }

    public String getPendidikan() {
        return mPendidikan;
    }

    public void setPendidikan(String pendidikan) {
        mPendidikan = pendidikan;
    }

    public String getPus4t() {
        return mPus4t;
    }

    public void setPus4t(String pus4t) {
        mPus4t = pus4t;
    }

    public String getTglLahir() {
        return mTglLahir;
    }

    public void setTglLahir(String tglLahir) {
        mTglLahir = tglLahir;
    }

    public String getUmur() {
        return mUmur;
    }

    public void setUmur(String umur) {
        mUmur = umur;
    }

}
